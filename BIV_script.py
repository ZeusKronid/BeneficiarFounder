...
class BeneficiarFounder():
    ...
    To load result in same folder with script call .download_result() the result will be saved in .tsv format
    ...
    def _init_owners_dict(self, ): 
        ...
        # natural_owners_df = (
        #     founder_natural_cleared_pl
        #     .group_by('company_id')
        #     .agg([pl.struct(['uid', 'share_percent']).alias('owners')])
        # )
        
        # self.natural_owners_dict = dict(zip(natural_owners_df['company_id'], natural_owners_df['owners']))
        
        # legal_owners_df = (
        #     founder_legal_cleared_pl
        #     .group_by('company_id')
        #     .agg([pl.struct(['uid', 'share_percent']).alias('owners')])
        # )

        # self.legal_owners_dict = dict(zip(legal_owners_df['company_id'], legal_owners_df['owners']))
    ...
    def _build_graph_for_node(self, company_id, company_df, founder_legal_df, founder_natural_df, G=None):
        ...
        while queue:
            ...
            # Добавляем основную компанию в граф
    ...
    def build_full_graph(self, ): 
        ...
        G = nx.Graph()  # Начальный пустой графDiGraph
    ...
    def draw_graph(self, draw_indexes): 
        """
        Строит и рисует граф владения для всех компаний.
        """
        G = nx.Graph()  # Начальный пустой графDiGraph
